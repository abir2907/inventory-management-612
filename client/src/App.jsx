...
const AppContent = () => {
  ...
  const [siteTemporarilyClosed, setSiteTemporarilyClosed] = useState(() => {
    const saved = localStorage.getItem("siteTemporarilyClosed");
    return saved ? JSON.parse(saved) : false;
  });

  // Save site closure state to localStorage whenever it changes
  useEffect(() => {
    localStorage.setItem(
      "siteTemporarilyClosed",
      JSON.stringify(siteTemporarilyClosed)
    );
  }, [siteTemporarilyClosed]);
  ...
  return user ? (
    <SnackInventoryApp
      ...
      setSiteTemporarilyClosed={setSiteTemporarilyClosed}
    />
  ) : (
    <AuthComponent />
  );
...
// TEMPORARILY CLOSE SITE
// import React, { useState, useEffect } from "react";
// import { Clock, AlertCircle, RefreshCw, Mail, Heart, Star } from "lucide-react";

// const TemporarilyClosedApp = () => {
